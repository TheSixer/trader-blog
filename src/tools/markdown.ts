...
// 段落解析
customRenderer.paragraph = (text: any) => `<p>${text}</p>`;
// 标题解析
customRenderer.heading = (text: any, level: any, raw: string) => {
  const id = raw.toLowerCase().replace(/[^a-zA-Z0-9\u4E00-\u9FA5]+/g, '-');
  return `<h${level} id=${id} alt=${id} title=${id}>${text}</h${level}>`;
};
// 对连接进行权重防流和新窗处理
customRenderer.link = (href: string, title: any, text: string) => {
...
// Image -> popup
customRenderer.image = (src: string, title: string, alt: string) => {
  // HTTP -> proxy
...
// 代码解析器（行号处理）
customRenderer.code = function getcode(
  code: string,
  lang: string,
  escaped: boolean
) {
  ...
  const lineNumbers = code
    .split('\n')
    .map((_, i) =>
      `<li class="code-line-number">${i + 1}</li>`.replace(/\s+/g, ' ')
    )
    .join('');
